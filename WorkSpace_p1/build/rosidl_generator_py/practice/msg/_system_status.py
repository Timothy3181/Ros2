...
class Metaclass_SystemStatus(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('practice')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'practice.msg.SystemStatus')
            logger.debug(
...
class SystemStatus(metaclass=Metaclass_SystemStatus):
    ...
    def __init__(self, **kwargs):
        ...
        self.cpu_available = kwargs.get('cpu_available', float())
    ...
    def __eq__(self, other):
        ...
        if self.cpu_percent != other.cpu_percent:
            return False
        if self.cpu_available != other.cpu_available:
            return False
    ...
    def cpu_available(self):
        """Message field 'cpu_available'."""
        return self._cpu_available

    @cpu_available.setter
    def cpu_available(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'cpu_available' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'cpu_available' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._cpu_available = value
